...
Diffuse_specular::Diffuse_specular(std::size_t exponent, float diffuse_weight, float diffuse_ratio): exponent(exponent), diffuse_weight(diffuse_weight), diffuse_ratio(diffuse_ratio)
{
    specular_weight = 1 - diffuse_weight;
}

float Diffuse_specular::get_specular_weight()
{
    return specular_weight;
}

float Diffuse_specular::get_diffuse_weight()
{
    return diffuse_weight;
}

Vector Diffuse_specular::get_color_light(Point_Light &light, Ray &ray, Vector &hit_point)
{
...
float Diffuse_specular::get_diffuse_ratio()
{
    return diffuse_ratio;
}

Vector Diffuse_specular::get_reflection_direction(Ray &ray, Vector &hit_point, Point_Light &light)
{
