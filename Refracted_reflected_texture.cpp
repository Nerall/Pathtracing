...
Ray Refracted_reflected_texture::create_refraction_ray(std::shared_ptr<Refracted_reflected_texture> texture, Ray const& ray, Vector const& hit_point)
{
    ...
    if (cos_incident < 0)
        refracted_origin = hit_point - 0.1 * ray.get_hit()->get_normal(hit_point);
    else
        refracted_origin = hit_point + 0.1 * ray.get_hit()->get_normal(hit_point);
...
Ray Refracted_reflected_texture::create_reflection_ray(std::shared_ptr<Refracted_reflected_texture> texture, Ray const& ray, Vector const& hit_point)
{
    ...
    if (cos_incident < 0)
        reflected_origin = hit_point + 0.1 * ray.get_hit()->get_normal(hit_point);
    else
        reflected_origin = hit_point - 0.1 * ray.get_hit()->get_normal(hit_point);
...
Vector Refracted_reflected_texture::get_refraction_direction(Ray const& ray, Vector const& hit_point)
{
    ...
    {
        return ((refraction_index * ray.get_direction()) + ((refraction_index * cos_incident) - sqrtf(cos_refracted2)) * normal).normalize();
    }
}
...
float Refracted_reflected_texture::get_fresnel_ratio(Ray const& ray, Vector const& hit_point)
{
    ...
    else
    {
        ...
        float parallel_light = powf(((coming_refraction_ratio * cos_refracted) - (leaving_refraction_ratio * cos_incident)) / ((coming_refraction_ratio * cos_refracted) + (leaving_refraction_ratio * cos_incident)), 2);

        float perpendicular_light = powf(((coming_refraction_ratio * cos_incident) - (leaving_refraction_ratio * cos_refracted)) / ((coming_refraction_ratio * cos_incident) + (leaving_refraction_ratio * cos_refracted)), 2);
